...
router.post("/login", jsonParser, function (req, res, next) {
  db.query("SELECT * FROM users WHERE email = ?", [req.body.email], function (err, users) {
    ...
    db.query("SELECT * FROM `likes` WHERE user_id = ?", [users[0].id], function (err, likeRows) {
      if (err) {
        res.json({ status: "error", message: err });
        return;
      }
      ...
      if (numOfLikes === 0) {
        db.query("SELECT id FROM articles", function (err, articleRows) {
          ...
          var values = articleIds.map((articleId) => [users[0].id, articleId]);
          db.query("INSERT INTO `likes` (user_id, article_id) VALUES ?", [values], function (err, result) {
            if (err) {
              res.json({ status: "error", message: err });
              return;
            }
      ...
      if (numOfLikes !== 0) {
        db.query("SELECT article_id FROM `likes` WHERE user_id = ?", [users[0].id], function (err, existingArticleRows) {
          if (err) {
            res.json({ status: "error", message: err });
            return;
          }

          const existingArticleIds = new Set(existingArticleRows.map((row) => row.article_id));
          ...
          db.query("SELECT id FROM articles", function (err, articleRows) {
            ...
            const newArticles = articleRows.filter((row) => !existingArticleIds.has(row.id));

            if (newArticles.length === 0) {
              return;
            }

            const values = newArticles.map((article) => [users[0].id, article.id]);
            db.query("INSERT INTO `likes` (user_id, article_id) VALUES ?", [values], function (err, result) {
              if (err) {
                res.json({ status: "error", message: err });
                return;
