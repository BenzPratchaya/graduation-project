...
exports.updateLikeForLike = (req, res) => {
  db.query("UPDATE `likes` SET `liked` = 1 WHERE article_id = ? AND user_id = ?", [req.params.article_id, req.params.user_id], (err, result) => {
    if (err) {
...
exports.updateLikeForUnLike = (req, res) => {
  db.query("UPDATE `likes` SET `liked` = 0 WHERE article_id = ? AND user_id = ?", [req.params.article_id, req.params.user_id], (err, result) => {
    if (err) {
